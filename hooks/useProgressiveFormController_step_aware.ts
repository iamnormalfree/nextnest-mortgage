...
export function useProgressiveFormController({
  ...
  const calculateInstant = useCallback((options: { force?: boolean; ltvMode?: number } = {}) => {
    ...
    if (mappedLoanType === 'new_purchase') {
      ...
      console.log('🔍 calculateInstant price check:', {
        rawPrice,
        propertyPrice,
        'priceRange from formData': formData.priceRange,
        'propertyPrice from formData': formData.propertyPrice
      })

      if (propertyPrice <= 0) {
        console.log('🔍 calculateInstant: propertyPrice <= 0, returning NULL')
        result = null
      } else {
        ...
        console.log('[DEBUG] InstantProfile calculation inputs:', {
          property_price: propertyPrice,
          rawPrice,
          priceRange: formData.priceRange,
          propertyPrice_field: formData.propertyPrice,
          property_type: personaPropertyType,
          buyer_profile: buyerProfile,
          existing_properties: existingProperties,
          formData_existingProperties: formData.existingProperties,
          income,
          commitments,
          rate: rateAssumption,
          tenure,
        ...
        console.log('[DEBUG] InstantProfile result:', {
