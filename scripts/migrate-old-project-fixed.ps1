...
if ($gitStatus) {
    Write-Host "  âœ— Error: Uncommitted changes detected" -ForegroundColor Red
...
if ($totalOldFiles -eq 0) {
    Write-Host "  â„¹ No old setup detected" -ForegroundColor Gray
...
if ($totalOldFiles -gt 0) {
    ...
    if ($DryRun) {
        ...
        foreach ($dir in $oldSkillDirs) {
            Write-Host "    - .claude\skills\$dir\ â†’ $skillArchivePath\$dir\" -ForegroundColor Gray
        }
        foreach ($file in $oldSkillFiles) {
            Write-Host "    - .claude\skills\$file â†’ $skillArchivePath\$file" -ForegroundColor Gray
        }
        foreach ($dir in $oldCommandDirs) {
            Write-Host "    - .claude\commands\$dir\ â†’ $commandArchivePath\$dir\" -ForegroundColor Gray
        }
    } else {
        # Archive skills
        if ($oldSkillDirs.Count -gt 0 -or $oldSkillFiles.Count -gt 0) {
...
if (Test-Path ".claude\frameworks\shared") {
    Write-Host "  âš  Warning: Subtree already exists at .claude\frameworks\shared" -ForegroundColor Yellow
    ...
    if ($overwrite -eq "y") {
        if ($DryRun) {
            ...
            Write-Host "  âœ“ Removed old subtree" -ForegroundColor Green
        }
    } else {
    ...
    Write-Host "  âœ“ No existing subtree" -ForegroundColor Green
}
Write-Host ""
...
if (-not $skipSubtree) {
    ...
    if ($DryRun) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  âœ“ Subtree added successfully" -ForegroundColor Green
        } else {
            Write-Host "  âœ— Error: Failed to add subtree" -ForegroundColor Red
            exit 1
        }
    }
} else {
...
if (Test-Path $commandPath) {
    if ($DryRun) {
        ...
        if ($content -match $oldPattern) {
            ...
            Write-Host "  âœ“ Updated path in response-awareness.md" -ForegroundColor Green
            Write-Host "  â„¹ Backup saved: $backupPath" -ForegroundColor Gray
        } else {
            Write-Host "  âš  Pattern not found - may need manual update" -ForegroundColor Yellow
            Write-Host "    Check if path is already correct or different" -ForegroundColor Gray
        }
    }
} else {
    Write-Host "  âš  Warning: response-awareness.md not found" -ForegroundColor Yellow
...
if ($missingConfigs.Count -gt 0) {
    ...
    if ($DryRun) {
        ...
        if (Test-Path ".claude\frameworks\shared\templates\config") {
            ...
            Write-Host "  âš  Templates not found - you'll need to create configs manually" -ForegroundColor Yellow
        }
    }
} else {
    Write-Host "  âœ“ All config files exist" -ForegroundColor Green
}
Write-Host ""
...
if (Test-Path ".claude\frameworks\shared\templates\agents\worktree-helper.md") {
    if (-not (Test-Path $agentPath)) {
        ...
        Write-Host "  âœ“ Exists: worktree-helper.md (keeping your version)" -ForegroundColor Gray
    }
} else {
...
if ($existingAgents) {
    Write-Host "  â„¹ Found existing agents (preserved):" -ForegroundColor Gray
...
"@
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  âš  Commit may have failed - check git status" -ForegroundColor Yellow
        }
    } else {
...
if ($DryRun) {
    ...
    if ($oldSkillDirs.Count -gt 0 -or $oldSkillFiles.Count -gt 0) {
        Write-Host "  âœ“ Archived old skills to .claude\skills\archive\2025-10-pre-subtree\" -ForegroundColor Gray
    }
    if ($oldCommandDirs.Count -gt 0) {
        Write-Host "  âœ“ Archived old commands to .claude\commands\archive\2025-10-pre-subtree\" -ForegroundColor Gray
    }
    if (-not $skipSubtree) {
        Write-Host "  âœ“ Added git subtree at .claude\frameworks\shared\" -ForegroundColor Gray
    }
    Write-Host "  âœ“ Updated configuration" -ForegroundColor Gray
    Write-Host "  âœ“ Committed changes" -ForegroundColor Gray
    Write-Host ""
