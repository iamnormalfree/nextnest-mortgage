...
echo "⏳ Step 4: Waiting for Dr. Elena response (max 10 seconds)..."
WAIT_TIME=0
MAX_WAIT=10
AI_RESPONSE=""
...
while [ $WAIT_TIME -lt $MAX_WAIT ]; do
  ...
  # Look for outgoing (AI) messages
  AI_RESPONSE=$(echo "$MESSAGES" | grep -o '"message_type":"outgoing".*"content":"[^"]*"' | grep -o '"content":"[^"]*"' | tail -1 | sed 's/"content":"//' | sed 's/"$//')
...
if echo "$AI_RESPONSE" | grep -qi "341,000\|341000"; then
