...
interface Step3NewPurchaseProps {
  ...
  fieldOrder?: 'income_first' | 'age_first'
}
...
export function Step3NewPurchase({ onFieldChange, showJointApplicant, errors, touchedFields, getErrorMessage, control, instantCalcResult, masReadiness, fieldOrder = 'income_first' }: Step3NewPurchaseProps) {
  ...
  // Helper function to render income fields based on employment type
  const renderIncomeFields = (isFirstSection = false) => (
    <>
      {/* Income field shows based on employment type */}
      {employmentType && (employmentType === 'employed' || employmentType === 'in-between-jobs') && (
        <Controller
          name="actualIncomes.0"
          control={control}
          render={({ field }) => (
            <div className={isFirstSection ? '' : 'mt-4'}>
      ...
      {employmentType && employmentType !== 'not-working' && employmentType !== 'self-employed' && (
        <div className={isFirstSection ? 'mt-4' : 'mt-4'}>
          <Controller
            name="actualVariableIncomes.0"
            control={control}
            render={({ field }) => (
              <div>
                <label htmlFor="variable-income-primary" className="text-xs uppercase tracking-wider text-[#666666] font-semibold mb-2 block">
                  Variable Income/Allowances (Optional)
                </label>
                <Input
                  {...field}
                  id="variable-income-primary"
                  type="text"
                  inputMode="numeric"
                  placeholder="0"
                  value={field.value ? formatNumberWithCommas(field.value.toString()) : ''}
  ...
  // Helper function to render age field
      ...
      <Controller
        ...
        render={({ field }) => (
          <div>
            ...
            />
            {errors['actualAges.0'] && (
              <p className="text-[#EF4444] text-xs mt-1">Age is required (21-75)</p>
            )}
          </div>
        )}
      />
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-black">
          {fieldOrder === 'income_first' ? 'Income Details' : 'Profile Details'}
        </h3>

        {!isPrimaryIncomeExpanded ? (
          // COLLAPSED SUMMARY VIEW
          <div className="p-4 border border-[#E5E5E5] border-l-4 border-l-[#FCD34D] bg-[#F8F8F8] flex justify-between items-center">
            <div>
            ...
            <button
              ...
              className="text-sm text-[#000000] hover:text-[#FCD34D] transition-colors"
            >
              Expand →
            </button>
          </div>
          ...
          <div className="p-4 border border-[#E5E5E5] border-l-4 border-l-[#FCD34D] bg-[#F8F8F8]">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                type="button"
                onClick={() => setIsPrimaryIncomeExpanded(false)}
                className="text-sm text-[#000000] hover:text-[#FCD34D] transition-colors"
              >
                Collapse ↑
              </button>
            ...
            {/* FIELD ORDER LOGIC: income_first vs age_first */}
            {fieldOrder === 'income_first' ? (
              // Income-first: Show income fields first, then age
              <>
                {renderIncomeFields(true)}
                {renderAgeField()}
              </>
            ) : (
              // Age-first: Show age field first, then income
              <>
                {renderAgeField()}
                {renderIncomeFields(false)}
              </>
            )}
          </div>
        )}
      ...
      {employmentType && primaryIncome > 0 && age > 0 && (
        <div className="space-y-4">
          ...
          <div className="space-y-6 p-4 border border-[#E5E5E5] border-l-4 border-l-[#FCD34D]">
