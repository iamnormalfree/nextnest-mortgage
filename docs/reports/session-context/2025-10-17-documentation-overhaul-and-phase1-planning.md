...
**Cleanup to Schedule (After Design Approved):**
- Archive `ProgressiveFormMobile.tsx` (incomplete parallel implementation)
- Delete abandoned design files (`/app/redesign/*` except `sophisticated-flow/`)
- Remove legacy form files (`ProgressiveForm.tsx`, `IntelligentMortgageForm.tsx`)
- Delete backup/patch files (`*.backup`, `*.fixed`, `*.patch`)
- Archive old experiments (`.NextNest_Mortgage_Calculator/`)
- Consolidate root-level documentation per new hierarchy
