---
title: frontend-backend-ai-architecture-ux
type: report
status: analysis
owner: engineering
date: 2025-09-02
...
interface FormData {
  ...
  step2: {
    ...
    propertyType?: string
    priceRange?: number
    combinedAge?: number  // Added for tenure calculations
  ...
  step3: {
    ...
    employmentType?: 'employed' | 'self_employed' | 'both_employed' | 'one_self_employed' | 'both_self_employed'
  }
}
...
export function ProgressiveForm({
  ...
  const renderStep3 = () => (
    <FieldLimiter maxVisible={2}>
      ...
      {/* Employment Type - For income recognition calculation */}
      {visibleFields.includes('employmentType') && (
        <div className="field-group animate-slideIn">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Employment Status
            <AIIndicator field="employmentType" />
          </label>
          <Controller
            name="employmentType"
            control={control}
            render={({ field }) => (
              <select
                ...
                onChange={(e) => {
                  field.onChange(e)
                  onFieldChange(3, 'employmentType', e.target.value)
                }}
              >
                <option value="">Select status</option>
                <option value="employed">Employed (100% income)</option>
                <option value="self_employed">Self-Employed (70% income)</option>
              </select>
            )}
          />
        </div>
...
interface ProgressiveDataFlow {
  ...
  step2: {
    fields: ['propertyCategory', 'propertyType', 'priceRange', 'combinedAge', 'purchaseTimeline'],
    apiCall: true,  // First API call after enough data
    insights: ['market_pulse', 'preliminary_calculation', 'tenure_analysis']
  },
  step3: {
    fields: ['applicationType', 'monthlyIncome/applicant1Income', 'applicant2Income?', 'monthlyCommitments', 'employmentType'],
    apiCall: true,  // Final API call with complete data
    insights: ['tdsr_analysis', 'msr_check', 'affordability_report', 'broker_prep']
  }
}
```
